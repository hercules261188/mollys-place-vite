import {
	createSelector,
	createSlice,
	PayloadAction,
} from '@reduxjs/toolkit';
import { IPost } from '../models';
import { RootState } from './store';

//
// Types...
type EditorSlice = {
	post: {
		background: string | undefined;
		content?: IPost['content'];
		errMsg: string;
		submission: string;
	};
};

//
// Initial state...
const initialState: EditorSlice = {
	post: {
		background: undefined,
		content: undefined,
		errMsg: ``,
		submission: ``,
	},
};

//
// Reducer...
export const editorSlice = createSlice({
	initialState,
	name: `editor`,
	reducers: {
		setPostBackground: (
			state,
			action: PayloadAction<IPost['background']>
		) => {
			state.post.background = action.payload;
		},
		setPostContent: (
			state,
			action: PayloadAction<IPost['content'] | undefined>
		) => {
			if (action.payload) {
				state.post.content = state.post.content
					? { ...state.post.content, ...action.payload }
					: action.payload;
			} else {
				state.post = initialState.post;
			}
		},
		setPostErrMsg: (state, action: PayloadAction<string>) => {
			state.post.errMsg = action.payload;
		},
		setPostSubmission: (state, action: PayloadAction<string>) => {
			state.post.submission = action.payload;
		},
	},
});

export const {
	setPostBackground,
	setPostContent,
	setPostErrMsg,
	setPostSubmission,
} = editorSlice.actions;
export const editorReducer = editorSlice.reducer;

//
// Selectors...
const _selectEditorSlice = (state: RootState) => state.editor;

const _selectPost = createSelector(
	[_selectEditorSlice],
	editor => editor.post
);

export const selectPostBackground = createSelector(
	[_selectPost],
	post => post.background
);

export const selectPostContent = createSelector(
	[_selectPost],
	post => post.content
);

export const selectPostErrMsg = createSelector(
	[_selectPost],
	post => post.errMsg
);

export const selectPostSubmission = createSelector(
	[_selectPost],
	post => post.submission
);
